...
def loc_counter_one_repo(owner, repo_name, data, cache_comment, history, addition_total, deletion_total, my_commits):
    """
    Recursively counts LOC for commits authored by the owner
    """
...
def loc_query(owner_affiliation, comment_size=0, force_cache=False, cursor=None, edges=None):
    """
    Queries all repositories accessible to the user and returns total lines of code
    """
    if edges is None:
        edges = []
    ...
    new_edges = edges + request.json()['data']['user']['repositories']['edges']
    if page_info['hasNextPage']:
        return loc_query(owner_affiliation, comment_size, force_cache, page_info['endCursor'], new_edges)
    else:
...
def cache_builder(edges, comment_size, force_cache):
    """
    Caches LOC count and commits per repo
    """
    ...
    for index in range(len(edges)):
        ...
        if repo_hash == hashlib.sha256(edges[index]['node']['nameWithOwner'].encode('utf-8')).hexdigest():
            try:
                ...
                if branch_ref and int(commit_count) != branch_ref['target']['history']['totalCount']:
...
def flush_cache(edges, filename, comment_size):
    """
    Wipes and resets the cache file
    """
    data = []
    if comment_size > 0:
...
def force_close_file(data, cache_comment):
    """
    Preserves whatever data was written before a crash
    """
    os.makedirs('cache', exist_ok=True)
    filename = 'cache/' + hashlib.sha256(USER_NAME.encode('utf-8')).hexdigest() + '.txt'
    with open(filename, 'w') as f:
...
def justify_format(root, element_id, new_text, length=0):
    """
    Updates and formats element text, adjusting dot count for alignment
    """
...
def find_and_replace(root, element_id, new_text):
    """
    Replaces text inside SVG element matching ID
    """
...
def commit_counter(comment_size):
    """
    Counts total commits using the cache file
    """
    total_commits = 0
    filename = 'cache/' + hashlib.sha256(USER_NAME.encode('utf-8')).hexdigest() + '.txt'
    if not os.path.exists(filename):
...
def user_getter(username):
    """
    Returns account ID and creation time
    """
...
if __name__ == '__main__':
    """
    Koushik Rai profile README stats generator
    """
    if not ACCESS_TOKEN:
        print("ACCESS_TOKEN environment variable not found. Please provide a GitHub Personal Access Token.")
        exit(0)
